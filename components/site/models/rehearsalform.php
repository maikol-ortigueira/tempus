 ...
 * @since  1.6
 */
class TempusModelRehearsalForm extends \Joomla\CMS\MVC\Model\FormModel
{
    private $item = null;
    ...
    protected function populateState()
    {
        ...
        {
                $id = Factory::getApplication()->getUserState('com_tempus.edit.rehearsal.id');
        }
        else
        {
                $id = Factory::getApplication()->input->get('id');
                Factory::getApplication()->setUserState('com_tempus.edit.rehearsal.id', $id);
        }

        $this->setState('rehearsal.id', $id);
        ...
        {
                $this->setState('rehearsal.id', $params_array['item_id']);
        }
    ...
    {
        // Get the id.
        $id = (!empty($id)) ? $id : (int) $this->getState('rehearsal.id');
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_tempus.rehearsal', 'rehearsalform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('rehearsal.id');
    ...
    public function delete($pk)
    {
        $user = Factory::getUser();
            ...
            if (empty($pk))
            {
                    $pk = (int) $this->getState('rehearsal.id');
            }
