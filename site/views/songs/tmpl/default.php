...
<form action="<?php echo htmlspecialchars(Uri::getInstance()->toString()); ?>" method="post"
	...
	<?php echo JHtml::_('grid.sort', 'JPUBLISHED', 'a.state', $listDirn, $listOrder); ?>
			...
			<?php endif; ?>
							...
							<th class=''>
				<?php echo JHtml::_('grid.sort',  'COM_TEMPUS_SONGS_ID', 'a.id', $listDirn, $listOrder); ?>
				</th>


							<?php if ($canEdit || $canDelete): ?>
					<th class="center">
				<?php echo JText::_('COM_TEMPUS_SONGS_ACTIONS'); ?>
				</th>
				<?php endif; ?>
		...
		<?php foreach ($this->items as $i => $item) : ?>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<?php if (isset($this->items[0]->state)) : ?>
					...
					<td class="center">
	<a class="btn btn-micro <?php echo $class; ?>" href="<?php echo ($canChange) ? JRoute::_('index.php?option=com_tempus&task=song.publish&id=' . $item->id . '&state=' . (($item->state + 1) % 2), false, 2) : '#'; ?>">
	<?php if ($item->state == 1): ?>
	...
	<?php if ($canCreate) : ?>
		<a href="<?php echo Route::_('index.php?option=com_tempus&task=songform.edit&id=0', false, 0); ?>"
