...
(function() {
	 ...
	 * View element calls jSelectRehearsal when an article is clicked
	 * jSelectRehearsal creates the link tag, sends it to the editor,
	 * and closes the select frame.
	 **/
	window.jSelectRehearsal = function (id, title, catid, object, link, lang) {
		var hreflang = '', editor, tag;

		if (!Joomla.getOptions('xtd-rehearsals')) {
			// Something went wrong!
			window.parent.jModalClose();
			return false;
		}

		editor = Joomla.getOptions('xtd-rehearsals').editor;
	...
	document.addEventListener('DOMContentLoaded', function(){
		...
		for(var i = 0, l = elements.length; l>i; i++) {
			...
			elements[i].addEventListener('click', function (event) {
				...
				if (functionName === 'jSelectRehearsal') {
